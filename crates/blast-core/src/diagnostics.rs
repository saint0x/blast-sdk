...
mod tests {
    ...
    fn test_diagnostic_creation() {
        ...
        .with_details("Could not find package 'foo' in PyPI".to_string())
        .with_suggestion(DiagnosticSuggestion {
            description: "Check package name".to_string(),
            fix: Some("blast search foo".to_string()),
            context: Some("Package names are case-sensitive".to_string()),
            estimated_time: Some("1 minute".to_string()),
            auto_fixable: false,
        })
        .with_code_context(CodeContext {
            file: PathBuf::from("src/resolver.rs"),
            line: Some(42),
            column: Some(10),
            snippet: Some("resolve_dependency(\"foo\")".to_string()),
            function: Some("resolve_dependency".to_string()),
        })
        .with_operation_context("Installing package 'foo'".to_string());

        assert_eq!(diagnostic.level, DiagnosticLevel::Error);
        assert_eq!(diagnostic.category, DiagnosticCategory::Dependency);
        assert!(diagnostic.suggestions.len() == 1);
    ...
    fn test_diagnostic_collection() {
        ...
        // Test filtering
        assert_eq!(collection.by_level(DiagnosticLevel::Error).len(), 2);
        assert_eq!(collection.by_level(DiagnosticLevel::Warning).len(), 1);
        assert_eq!(collection.by_category(&DiagnosticCategory::Dependency).len(), 2);
        assert_eq!(collection.by_category(&DiagnosticCategory::Version).len(), 1);
